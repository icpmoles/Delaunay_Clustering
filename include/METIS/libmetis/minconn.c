...
{
  idx_t i, ii, j, k, ncon, nparts, scheme, pid_from, pid_to, me, other, nvtxs, total, max, avg, totalout,
    nind = 0, ncand = 0, ncand2, target, target2, nadd, bestnadd = 0;
...
void MoveGroupMinConnForCut(ctrl_t* ctrl, graph_t* graph, idx_t to, idx_t nind, idx_t* ind)
{
  ...
  while(--nind >= 0)
  {
    ...
    for(j = xadj[i]; j < xadj[i + 1]; j++)
    {
      ...
      UpdateAdjacentVertexInfoAndBND(ctrl, ii, xadj[ii + 1] - xadj[ii], me, from, to, myrinfo, adjwgt[j], nbnd, bndptr,
