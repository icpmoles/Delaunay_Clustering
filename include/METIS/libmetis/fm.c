  ...
  { /* Do a number of passes */
    ...
    for(nswaps = 0; nswaps < nvtxs; nswaps++)
    {
      ...
      if((newcut < mincut && newbal <= ffactor) ||
         (newcut == mincut && (newbal < minbal || (newbal == minbal && BetterBalance2Way(ncon, minbalv, newbalv)))))
      {
        mincut = newcut;
        minbal = newbal;
        mincutorder = nswaps;
      ...
      if(ctrl->dbglvl & METIS_DBG_MOVEINFO)
      {
        ...
        printf(" %+.3" PRREAL " LB: %.3" PRREAL "(%+.3" PRREAL ")\n", minbal,
               ComputeLoadImbalance(graph, 2, ctrl->pijbm), newbal);
      }
...
void Print2WayRefineStats(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts, real_t deltabal, idx_t mincutorder)
{
  ...
  if(mincutorder == -2)
  {
    ...
    for(i = 0; i < graph->ncon; i++)
      printf("(%.3" PRREAL " %.3" PRREAL " T:%.3" PRREAL " %.3" PRREAL ")", graph->pwgts[i] * graph->invtvwgt[i],
             graph->pwgts[graph->ncon + i] * graph->invtvwgt[i], ntpwgts[i], ntpwgts[graph->ncon + i]);
  ...
  else
  {
    ...
    for(i = 0; i < graph->ncon; i++)
      printf("(%.3" PRREAL " %.3" PRREAL ")", graph->pwgts[i] * graph->invtvwgt[i],
