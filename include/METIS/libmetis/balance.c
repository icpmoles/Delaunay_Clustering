...
void Bnd2WayBalance(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts)
{
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_REFINE,
        printf("Partitions: [%6" PRIDX " %6" PRIDX "] T[%6" PRIDX " %6" PRIDX "], Nv-Nb[%6" PRIDX " %6" PRIDX
               "]. ICut: %6" PRIDX " [B]\n",
...
void McGeneral2WayBalance(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts)
{
  ...
  if(ctrl->dbglvl & METIS_DBG_REFINE)
  {
    printf("Parts: [");
    for(l = 0; l < ncon; l++)
      printf("(%6" PRIDX " %6" PRIDX " %.3" PRREAL " %.3" PRREAL ") ", pwgts[l], pwgts[ncon + l], ntpwgts[l],
             ntpwgts[ncon + l]);
    printf("] Nv-Nb[%5" PRIDX ", %5" PRIDX "]. ICut: %6" PRIDX ", LB: %+.3" PRREAL " [B]\n", graph->nvtxs, graph->nbnd,
           graph->mincut, minbal);
  }
  ...
  for(nswaps = 0; nswaps < nvtxs; nswaps++)
  {
    ...
    if(newbal < minbal ||
       (newbal == minbal && (newcut < mincut || (newcut == mincut && BetterBalance2Way(ncon, minbalv, newbalv)))))
    {
      mincut = newcut;
      minbal = newbal;
      mincutorder = nswaps;
