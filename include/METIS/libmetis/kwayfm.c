...
void Greedy_KWayCutOptimize(ctrl_t* ctrl, graph_t* graph, idx_t niter, real_t ffactor, idx_t omode)
{
  ...
  if(ctrl->dbglvl & METIS_DBG_REFINE)
  {
    printf("%s: [%6" PRIDX " %6" PRIDX "]-[%6" PRIDX " %6" PRIDX "], Bal: %5.3" PRREAL ","
           ...
           (omode == OMODE_REFINE ? "GRC" : "GBC"), pwgts[iargmin(nparts, pwgts, 1)], imax(nparts, pwgts, 1),
           minpwgts[0], maxpwgts[0], ComputeLoadImbalance(graph, nparts, ctrl->pijbm), graph->nvtxs, graph->nbnd,
           graph->mincut);
    if(ctrl->minconn)
  ...
  for(pass = 0; pass < niter; pass++)
  {
    ...
    for(nmoved = 0, iii = 0;; iii++)
    {
      ...
      if(omode == OMODE_REFINE)
      {
        for(k = myrinfo->nnbrs - 1; k >= 0; k--)
        {
          ...
          if(((mynbrs[k].ed > myrinfo->id) &&
              ...
              ((pwgts[to] + vwgt <= maxpwgts[to]) || (tpwgts[from] * pwgts[to] < tpwgts[to] * (pwgts[from] - vwgt)))) ||
             ((mynbrs[k].ed == myrinfo->id) && (tpwgts[from] * pwgts[to] < tpwgts[to] * (pwgts[from] - vwgt))))
            break;
        }
        if(k < 0)
      ...
      IFSET(ctrl->dbglvl, METIS_DBG_MOVEINFO,
            printf("\t\tMoving %6" PRIDX " from %3" PRIDX "/%" PRIDX " to %3" PRIDX "/%" PRIDX " [%6" PRIDX " %6" PRIDX
                   "]. Gain: %4" PRIDX ". Cut: %6" PRIDX "\n",
                   i, from, safetos[from], to, safetos[to], pwgts[from], pwgts[to], mynbrs[k].ed - myrinfo->id,
                   graph->mincut));
      ...
      for(j = xadj[i]; j < xadj[i + 1]; j++)
      {
        ...
        UpdateAdjacentVertexInfoAndBND(ctrl, ii, xadj[ii + 1] - xadj[ii], me, from, to, myrinfo, adjwgt[j], nbnd,
    ...
    if(ctrl->dbglvl & METIS_DBG_REFINE)
    {
      printf("\t[%6" PRIDX " %6" PRIDX "], Bal: %5.3" PRREAL ", Nb: %6" PRIDX "."
             ...
             pwgts[iargmin(nparts, pwgts, 1)], imax(nparts, pwgts, 1), ComputeLoadImbalance(graph, nparts, ctrl->pijbm),
...
void Greedy_KWayVolOptimize(ctrl_t* ctrl, graph_t* graph, idx_t niter, real_t ffactor, idx_t omode)
{
  ...
  if(ctrl->dbglvl & METIS_DBG_REFINE)
  {
    printf("%s: [%6" PRIDX " %6" PRIDX "]-[%6" PRIDX " %6" PRIDX "], Bal: %5.3" PRREAL ", Nv-Nb[%6" PRIDX " %6" PRIDX
           "], Cut: %5" PRIDX ", Vol: %5" PRIDX,
           (omode == OMODE_REFINE ? "GRV" : "GBV"), pwgts[iargmin(nparts, pwgts, 1)], imax(nparts, pwgts, 1),
           minpwgts[0], maxpwgts[0], ComputeLoadImbalance(graph, nparts, ctrl->pijbm), graph->nvtxs, graph->nbnd,
    ...
    for(nmoved = 0, iii = 0;; iii++)
    {
      ...
      if(omode == OMODE_REFINE)
      {
        ...
        for(j = k - 1; j >= 0; j--)
        {
          ...
          if((mynbrs[j].gv > mynbrs[k].gv && pwgts[to] + vwgt <= maxpwgts[to] + ffactor * gain) ||
             (mynbrs[j].gv == mynbrs[k].gv && mynbrs[j].ned > mynbrs[k].ned && pwgts[to] + vwgt <= maxpwgts[to]) ||
      ...
      { /* OMODE_BALANCE */
        for(k = myrinfo->nnbrs - 1; k >= 0; k--)
        {
          if(!safetos[to = mynbrs[k].pid])
            continue;
          if(pwgts[to] + vwgt <= maxpwgts[to] || tpwgts[from] * (pwgts[to] + vwgt) <= tpwgts[to] * pwgts[from])
            break;
        }
        if(k < 0)
          continue; /* break out if you did not find a candidate */
...
void Greedy_McKWayCutOptimize(ctrl_t* ctrl, graph_t* graph, idx_t niter, real_t ffactor, idx_t omode)
{
  ...
  if(ctrl->dbglvl & METIS_DBG_REFINE)
  {
    printf("%s: [%6" PRIDX " %6" PRIDX " %6" PRIDX "], Bal: %5.3" PRREAL "(%.3" PRREAL "),"
           ...
           (omode == OMODE_REFINE ? "GRC" : "GBC"), imin(nparts * ncon, pwgts, 1), imax(nparts * ncon, pwgts, 1),
           imax(nparts * ncon, maxpwgts, 1), ComputeLoadImbalance(graph, nparts, pijbm), origbal, graph->nvtxs,
  ...
  for(pass = 0; pass < niter; pass++)
  {
    ...
    for(nmoved = 0, iii = 0;; iii++)
    {
      ...
      if(omode == OMODE_REFINE)
      {
        if(myrinfo->id > 0 && !ivecaxpygez(ncon, -1, vwgt + i * ncon, pwgts + from * ncon, minpwgts + from * ncon))
          continue;
      }
      else
      { /* OMODE_BALANCE */
      ...
      if(omode == OMODE_REFINE)
      {
        ...
        for(j = k - 1; j >= 0; j--)
        {
          ...
          if((mynbrs[j].ed > mynbrs[k].ed &&
             ...
             (mynbrs[j].ed == mynbrs[k].ed &&
              BetterBalanceKWay(ncon, vwgt + i * ncon, ubfactors, 1, pwgts + cto * ncon, pijbm + cto * ncon, 1,
        ...
        if(!(gain > 0 ||
             (gain == 0 &&
              (BetterBalanceKWay(ncon, vwgt + i * ncon, ubfactors, -1, pwgts + from * ncon, pijbm + from * ncon, +1,
                                 pwgts + to * ncon, pijbm + to * ncon) ||
               (iii % 2 == 0 && safetos[to] == 2)))))
          continue;
      }
      ...
      { /* OMODE_BALANCE */
        ...
        for(j = k - 1; j >= 0; j--)
        {
          if(!safetos[to = mynbrs[j].pid])
            continue;
          if(BetterBalanceKWay(ncon, vwgt + i * ncon, ubfactors, 1, pwgts + cto * ncon, pijbm + cto * ncon, 1,
                               pwgts + to * ncon, pijbm + to * ncon))
          {
            k = j;
            cto = to;
      ...
      IFSET(ctrl->dbglvl, METIS_DBG_MOVEINFO,
            printf("\t\tMoving %6" PRIDX " to %3" PRIDX ". Gain: %4" PRIDX ". Cut: %6" PRIDX "\n", i, to,
                   mynbrs[k].ed - myrinfo->id, graph->mincut));
    ...
    if(ctrl->dbglvl & METIS_DBG_REFINE)
    {
      printf("\t[%6" PRIDX " %6" PRIDX "], Bal: %5.3" PRREAL ", Nb: %6" PRIDX "."
             " Nmoves: %5" PRIDX ", Cut: %6" PRIDX ", Vol: %6" PRIDX,
             imin(nparts * ncon, pwgts, 1), imax(nparts * ncon, pwgts, 1), ComputeLoadImbalance(graph, nparts, pijbm),
             graph->nbnd, nmoved, graph->mincut, ComputeVolume(graph, where));
      if(ctrl->minconn)
        printf(", Doms: [%3" PRIDX " %4" PRIDX "]", imax(nparts, nads, 1), isum(nparts, nads, 1));
      printf("\n");
...
void Greedy_McKWayVolOptimize(ctrl_t* ctrl, graph_t* graph, idx_t niter, real_t ffactor, idx_t omode)
{
  ...
  if(ctrl->dbglvl & METIS_DBG_REFINE)
  {
    printf("%s: [%6" PRIDX " %6" PRIDX " %6" PRIDX "], Bal: %5.3" PRREAL "(%.3" PRREAL "),"
           ...
           (omode == OMODE_REFINE ? "GRV" : "GBV"), imin(nparts * ncon, pwgts, 1), imax(nparts * ncon, pwgts, 1),
           imax(nparts * ncon, maxpwgts, 1), ComputeLoadImbalance(graph, nparts, pijbm), origbal, graph->nvtxs,
  ...
  for(pass = 0; pass < niter; pass++)
  {
    ...
    for(nmoved = 0, iii = 0;; iii++)
    {
      ...
      if(omode == OMODE_REFINE)
      {
        if(myrinfo->nid > 0 && !ivecaxpygez(ncon, -1, vwgt + i * ncon, pwgts + from * ncon, minpwgts + from * ncon))
          continue;
      }
      else
      { /* OMODE_BALANCE */
...
*/
/*************************************************************************/
void KWayVolUpdate(ctrl_t* ctrl, graph_t* graph, idx_t v, idx_t from, idx_t to, ipq_t* queue, idx_t* vstatus,
                   idx_t* r_nupd, idx_t* updptr, idx_t* updind, idx_t bndtype, idx_t* vmarker, idx_t* pmarker,
                   idx_t* modind)
{
...
void Greedy_KWayEdgeStats(ctrl_t* ctrl, graph_t* graph)
{
  ...
  for(ii = 0; ii < nbnd; ii++)
  {
    ...
    for(j = xadj[u]; j < xadj[u + 1]; j++)
    {
      ...
      if(uw == vw)
        continue;
      if(pwgts[uw] - vwgt[u] + vwgt[v] > maxpwgts[uw] || pwgts[vw] - vwgt[v] + vwgt[u] > maxpwgts[vw])
        continue;
...
void Greedy_KWayEdgeCutOptimize(ctrl_t* ctrl, graph_t* graph, idx_t niter)
{
  ...
  if(ctrl->dbglvl & METIS_DBG_REFINE)
  {
    printf("GRE: [%6" PRIDX " %6" PRIDX "]-[%6" PRIDX " %6" PRIDX "], Bal: %5.3" PRREAL ","
           ...
           ComputeLoadImbalance(graph, nparts, ctrl->pijbm), graph->nvtxs, graph->nbnd, graph->mincut);
  }
  ...
  for(pass = 0; pass < niter; pass++)
  {
    ...
    for(ii = 0; ii < nvtxs; ii++)
    {
      ...
      for(j = xadj[u]; j < xadj[u + 1]; j++)
      {
        ...
        if(pwgts[uw] - vwgt[u] + vwgt[v] > maxpwgts[uw] || pwgts[vw] - vwgt[v] + vwgt[u] > maxpwgts[vw])
          continue;
        if(pwgts[uw] - vwgt[u] + vwgt[v] < minpwgts[uw] || pwgts[vw] - vwgt[v] + vwgt[u] < minpwgts[vw])
          continue;
      ...
      IFSET(ctrl->dbglvl, METIS_DBG_MOVEINFO,
            printf("\t\tMoving %6" PRIDX " from %3" PRIDX " to %3" PRIDX " [%6" PRIDX " %6" PRIDX "]. Gain: %4" PRIDX
                   ". Cut: %6" PRIDX "\n",
