...
int main(int argc, char* argv[])
{
  ...
  case METIS_GTYPE_DUAL:
    status = METIS_PartMeshDual(&mesh->ne, &mesh->nn, mesh->eptr, mesh->eind, mesh->ewgt, NULL, &params->ncommon,
                                &params->nparts, params->tpwgts, options, &objval, epart, npart);
    break;

  case METIS_GTYPE_NODAL:
    status = METIS_PartMeshNodal(&mesh->ne, &mesh->nn, mesh->eptr, mesh->eind, NULL, NULL, &params->nparts,
...
void MPPrintInfo(params_t* params, mesh_t* mesh)
{
  ...
  printf(" dbglvl=%" PRIDX ", ufactor=%.3f, minconn=%s, contig=%s, nooutput=%s\n", params->dbglvl,
         I2RUBFACTOR(params->ufactor), (params->minconn ? "YES" : "NO"), (params->contig ? "YES" : "NO"),
  ...
  printf(" gtype=%s, ncommon=%" PRIDX ", niter=%" PRIDX ", ncuts=%" PRIDX "\n", gtypenames[params->gtype],
         params->ncommon, params->niter, params->ncuts);
