...
int main(int argc, char* argv[])
{
  ...
  case METIS_PTYPE_RB:
    status =
      METIS_PartGraphRecursive(&graph->nvtxs, &graph->ncon, graph->xadj, graph->adjncy, graph->vwgt, graph->vsize,
                               graph->adjwgt, &params->nparts, params->tpwgts, params->ubvec, options, &objval, part);
    break;

  case METIS_PTYPE_KWAY:
    status = METIS_PartGraphKway(&graph->nvtxs, &graph->ncon, graph->xadj, graph->adjncy, graph->vwgt, graph->vsize,
                                 graph->adjwgt, &params->nparts, params->tpwgts, params->ubvec, options, &objval, part);
    break;
  }
  ...
  gk_free((void**)&params->filename, &params->tpwgtsfile, &params->tpwgts, &params->ubvecstr, &params->ubvec, &params,
          LTERM);
}
...
void GPPrintInfo(params_t* params, graph_t* graph)
{
  ...
  printf(" dbglvl=%" PRIDX ", ufactor=%.3f, no2hop=%s, minconn=%s, contig=%s\n", params->dbglvl,
         I2RUBFACTOR(params->ufactor), (params->no2hop ? "YES" : "NO"), (params->minconn ? "YES" : "NO"),
         (params->contig ? "YES" : "NO"));

  printf(" ondisk=%s, nooutput=%s\n", (params->ondisk ? "YES" : "NO"), (params->nooutput ? "YES" : "NO"));

  printf(" seed=%" PRIDX ", niparts=%" PRIDX ", niter=%" PRIDX ", ncuts=%" PRIDX "\n", params->seed, params->niparts,
         params->niter, params->ncuts);
