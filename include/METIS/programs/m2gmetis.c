...
int main(int argc, char* argv[])
{
  ...
  case METIS_GTYPE_DUAL:
    status = METIS_MeshToDual(&mesh->ne, &mesh->nn, mesh->eptr, mesh->eind, &params->ncommon, &params->numflag,
  ...
  case METIS_GTYPE_NODAL:
    status =
      METIS_MeshToNodal(&mesh->ne, &mesh->nn, mesh->eptr, mesh->eind, &params->numflag, &graph->xadj, &graph->adjncy);
