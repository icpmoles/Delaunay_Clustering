 ...
 *-------------------------------------------------------------------*/
static struct gk_option long_options[] = {{"ptype", 1, 0, METIS_OPTION_PTYPE},
...
static gk_StringMap_t iptype_options[] = {
  {"grow", METIS_IPTYPE_GROW}, {"random", METIS_IPTYPE_RANDOM}, {"rb", METIS_IPTYPE_METISRB}, {NULL, 0}};
...
static char helpstr[][100] = {
  " ", "Usage: gpmetis [options] graphfile nparts", " ", " Required parameters",
  "    graphfile   Stores the graph to be partitioned.", "    nparts      The number of partitions to split the graph.",
  ...
  "     The possible values are:", "        rm       - Random matching",
  "        shem     - Sorted heavy-edge matching [default]", " ", "  -iptype=string [applies only when -ptype=rb]",
  "     Specifies the scheme to be used to compute the initial partitioning", "     of the graph.",
  "     The possible values are:", "        grow     - Grow a bisection using a greedy scheme [default for ncon=1]",
  ...
  */
  ...
  "  -contig [applies only when -ptype=kway]", "     Specifies that the partitioning routines should try to produce",
  "     partitions that are contiguous. Note that if the input graph is not", "     connected this option is ignored.",
  ...
  "     partition is a node, and edges connect subdomains with a shared", "     interface.", " ", "  -tpwgts=filename",
  "     Specifies the name of the file that stores the target weights for",
  "     each partition. By default, all partitions are assumed to be of ", "     the same size.", " ", "  -ufactor=int",
  ...
  "     For ptype=kway, the default value is 30 (i.e., load imbalance of 1.03).", " ", "  -ubvec=string",
  ...
  "     If supplied, this parameter takes priority over ufactor.", " ", "  -niparts=int",
  "     Specifies the number of initial partitions to compute. The default",
  "     value is determined by the algorithm automatically.", " ", "  -niter=int",
  ...
  "     communication volume. Default is 1.", " ", "  -ondisk",
  "     Specifies if graphs will be stored to disk during coarsening in order", "     to reduce memory requirements.",
  ...
  " ",
  */
  "  -seed=int", "     Selects the seed of the random number generator.  ", " ", "  -dbglvl=int      ",
...
params_t* parse_cmdline(int argc, char* argv[])
{
  ...
  while((c = gk_getopt_long_only(argc, argv, "", long_options, &option_index)) != -1)
  {
    ...
    case METIS_OPTION_NIPARTS:
      if(gk_optarg)
        params->niparts = (idx_t)atoi(gk_optarg);
      break;
