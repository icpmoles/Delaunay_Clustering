...
void TestParMetis(char* filename, MPI_Comm comm)
{
  ...
  for(nparts = 2 * npes; nparts >= npes / 2 && nparts > 0; nparts = nparts / 2)
  {
    for(ncon = 1; ncon <= 5; ncon += 2)
    {
      ...
      for(opt2 = 1; opt2 <= 2; opt2++)
      {
        ...
        if(mype == 0)
          printf("\nTesting ParMETIS_V3_PartKway with options[1-2] = {%" PRIDX " %" PRIDX "}, Ncon: %" PRIDX
                 ", Nparts: %" PRIDX "\n",
                 options[1], options[2], ncon, nparts);

        ParMETIS_V3_PartKway(graph.vtxdist, graph.xadj, graph.adjncy, graph.vwgt, NULL, &wgtflag, &numflag, &ncon,
  ...
  for(nparts = 2 * npes; nparts >= npes / 2 && nparts > 0; nparts = nparts / 2)
  {
    for(ncon = 1; ncon <= 5; ncon += 2)
    {
      ...
      for(opt2 = 1; opt2 <= 2; opt2++)
      {
        ...
        rset(nparts * ncon, 1.0 / (real_t)nparts, tpwgts);
        if(mype == 0)
          printf("\nTesting ParMETIS_V3_PartGeomKway with options[1-2] = {%" PRIDX " %" PRIDX "}, Ncon: %" PRIDX
                 ", Nparts: %" PRIDX "\n",
                 options[1], options[2], ncon, nparts);

        ParMETIS_V3_PartGeomKway(graph.vtxdist, graph.xadj, graph.adjncy, graph.vwgt, NULL, &wgtflag, &numflag, &ndims,
                                 xyz, &ncon, &nparts, tpwgts, ubvec, options, &edgecut, part, &comm);
  ...
  */
  ...
  ParMETIS_V3_RefineKway(graph.vtxdist, graph.xadj, graph.adjncy, NULL, NULL, &wgtflag, &numflag, &ncon, &nparts,
  ...
  ncon = 1;
  rset(nparts * ncon, 1.0 / (real_t)nparts, tpwgts);
  ParMETIS_V3_PartKway(graph.vtxdist, graph.xadj, graph.adjncy, NULL, NULL, &wgtflag, &numflag, &ncon, &npes, tpwgts,
  ...
  for(ncon = 1; ncon <= 5; ncon += 2)
  {
    for(opt2 = 1; opt2 <= 2; opt2++)
    {
      ...
      rset(nparts * ncon, 1.0 / (real_t)nparts, tpwgts);
      if(mype == 0)
        printf("\nTesting ParMETIS_V3_RefineKway with options[1-3] = {%" PRIDX " %" PRIDX " %" PRIDX "}, Ncon: %" PRIDX
               ", Nparts: %" PRIDX "\n",
               options[1], options[2], options[3], ncon, nparts);
      ParMETIS_V3_RefineKway(mgraph.vtxdist, mgraph.xadj, mgraph.adjncy, NULL, NULL, &wgtflag, &numflag, &ncon, &nparts,
                             tpwgts, ubvec, options, &edgecut, mpart, &comm);
  ...
  for(nparts = 2 * npes; nparts >= npes / 2; nparts = nparts / 2)
  {
    ...
    options[0] = 0;
    rset(nparts * ncon, 1.0 / (real_t)nparts, tpwgts);
    ParMETIS_V3_PartKway(mgraph.vtxdist, mgraph.xadj, mgraph.adjncy, NULL, NULL, &wgtflag, &numflag, &ncon, &nparts,
                         tpwgts, ubvec, options, &edgecut, savepart, &comm);
    options[0] = 1;
    wgtflag = 2;
    ...
    for(ncon = 1; ncon <= 3; ncon += 2)
    {
      ...
      for(ipc2redist = 1000.0; ipc2redist >= 0.001; ipc2redist /= 1000.0)
      {
        for(opt2 = 1; opt2 <= 2; opt2++)
        {
          ...
          if(mype == 0)
            printf("\nTesting ParMETIS_V3_AdaptiveRepart with options[1-3] = {%" PRIDX " %" PRIDX " %" PRIDX
                   "}, ipc2redist: %.3" PRREAL ", Ncon: %" PRIDX ", Nparts: %" PRIDX "\n",
                   options[1], options[2], options[3], ipc2redist, ncon, nparts);

          ParMETIS_V3_AdaptiveRepart(mgraph.vtxdist, mgraph.xadj, mgraph.adjncy, mgraph.vwgt, mgraph.vsize, NULL,
                                     &wgtflag, &numflag, &ncon, &nparts, tpwgts, ubvec, &ipc2redist, options, &edgecut,
                                     mpart, &comm);
  ...
  for(opt2 = 1; opt2 <= 2; opt2++)
  {
    ...
    ParMETIS_V3_NodeND(graph.vtxdist, graph.xadj, graph.adjncy, &numflag, options, order, sizes, &comm);
  }
 ...
 *****************************************************************************/
graph_t* SetUpGraph(ctrl_t* ctrl, idx_t* vtxdist, idx_t* xadj, idx_t* vwgt, idx_t* adjncy, idx_t* adjwgt, idx_t wgtflag)
{
  idx_t mywgtflag;
