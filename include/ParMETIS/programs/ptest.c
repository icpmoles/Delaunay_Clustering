...
void TestParMetis_GPart(char* filename, char* xyzfile, MPI_Comm comm)
{
  ...
  for(nparts = 2 * npes; nparts >= npes / 2 && nparts > 0; nparts = nparts / 2)
  {
    for(ncon = 1; ncon <= NCON; ncon++)
    {
      ...
      ParMETIS_V3_RefineKway(graph.vtxdist, graph.xadj, graph.adjncy, graph.vwgt, NULL, &wgtflag, &numflag, &ncon,
                             &nparts, tpwgts, ubvec, options, &edgecut, part, &comm);
  ...
  for(ncon = 1; ncon <= NCON; ncon++)
  {
    ...
    realcut = ComputeRealCutFromMoved(graph.vtxdist, mgraph.vtxdist, part, mpart, filename, comm);
    if(mype == 0)
      printf("ParMETIS_V3_RefineKway reported a cut of %" PRIDX " [%s:%" PRIDX "]\n", edgecut,
             (edgecut == realcut ? "OK" : "ERROR"), realcut);
  }
  ...
  for(nparts = 2 * npes; nparts >= npes / 2; nparts = nparts / 2)
  {
    ...
    for(ncon = 1; ncon <= NCON; ncon++)
    {
      ...
      for(ipc2redist = 1000.0; ipc2redist >= 0.001; ipc2redist /= 1000.0)
      {
        ...
        if(mype == 0)
          printf("\nTesting ParMETIS_V3_AdaptiveRepart with ipc2redist: %.3" PRREAL ", ncon: %" PRIDX
                 ", nparts: %" PRIDX "\n",
...
/*! This function takes a partition vector of the original graph and that of the
    ...
    graph */
/*******************************************************************************/
idx_t ComputeRealCutFromMoved(idx_t* vtxdist, idx_t* mvtxdist, idx_t* part, idx_t* mpart, char* filename, MPI_Comm comm)
{
  idx_t i, j, nvtxs, mype, npes, cut;
 ...
 *****************************************************************************/
graph_t* TestSetUpGraph(ctrl_t* ctrl, idx_t* vtxdist, idx_t* xadj, idx_t* vwgt, idx_t* adjncy, idx_t* adjwgt,
                        idx_t wgtflag)
{
