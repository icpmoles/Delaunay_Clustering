...
#define ToCyclicMap(id, nbuckets, bucketsize) (((id) % (nbuckets)) * (bucketsize) + (id) / (nbuckets))
/*
#define ToCyclicMap(id, nbuckets, bucketdist) \
            ((bucketdist)[(id)%(nbuckets)] + (id)/(nbuckets))
*/
/* (u%lnvtxs)*npes + u/lnvtxs */
#define FromCyclicMap(id, nbuckets, bucketsize) (((id) % (bucketsize)) * (nbuckets) + (id) / (bucketsize))
...
int DistDGL_GPart(char* fstem, idx_t nparts_per_pe, MPI_Comm comm)
{
  ...
  for(i = npes - 1; i >= 0; i--)
  {
    ...
    if(i == mype)
      printf("[%03" PRIDX "] proc/self/stat/VmPeak:     %.2f MB\n", mype,
             (float)gk_GetProcVmPeak() / (1024.0 * 1024.0));
    if(i == 0 && mype == 0)
      printf("-------------------------------------------------------\n");
    fflush(stdout);
    gkMPI_Barrier(comm);
  ...
  options[0] = 1;
  options[1] =
    15 + (PARMETIS_DBGLVL_TWOHOP | PARMETIS_DBGLVL_FAST | PARMETIS_DBGLVL_DROPEDGES | PARMETIS_DBGLVL_ONDISK);
  options[2] = 1;
  wgtflag = 2;
  numflag = 0;
  edgecut = 0;
  ...
  if(mype == 0)
    printf("\nDistDGL partitioning, ncon: %" PRIDX ", nparts: %" PRIDX " [%s, %s, MPI %d.%d]\n", graph->ncon, nparts,
           (sizeof(idx_t) == 8 ? "i64" : "i32"), (sizeof(real_t) == 8 ? "r64" : "r32"), MPI_VERSION, MPI_SUBVERSION);

  ParMETIS_V3_PartKway(graph->vtxdist, graph->xadj, graph->adjncy, graph->vwgt, NULL, &wgtflag, &numflag,
                       &(graph->ncon), &nparts, tpwgts, ubvec, options, &edgecut, part, &comm);
...
graph_t* DistDGL_MoveGraph(graph_t* ograph, idx_t* part, idx_t nparts_per_pe, MPI_Comm comm)
{
  ...
  graph =
    SetupGraph(ctrl, 1, ograph->vtxdist, ograph->xadj, ograph->vwgt, ograph->vsize, ograph->adjncy, ograph->adjwgt, 0);
  ...
  gkMPI_Alltoall((void*)sinfo, nparts_per_pe * (sizeof(mvinfo_t) / idxwidth), IDX_T, (void*)rinfo,
                 nparts_per_pe * (sizeof(mvinfo_t) / idxwidth), IDX_T, ctrl->comm);

  /* Use lpwgts/gpwgts as pointers to where data will be sent/received, respectively */
  for(nsnbrs = 0, i = 0; i < nparts; i++)
  {
    lpwgts[i] =
      3 * sinfo[i].nvtxs + sinfo[i].nedges + sinfo[i].nvtxs + sinfo[i].nvmdata + sinfo[i].nedges + sinfo[i].nemdata;
    if(sinfo[i].nvtxs > 0)
      nsnbrs++;
  }
  ...
  for(nrnbrs = 0, k = 0; k < nparts_per_pe; k++)
  {
    for(i = 0; i < npes; i++)
    {
      gpwgts[k * npes + i] = 3 * rinfo[i * nparts_per_pe + k].nvtxs + rinfo[i * nparts_per_pe + k].nedges +
        rinfo[i * nparts_per_pe + k].nvtxs + rinfo[i * nparts_per_pe + k].nvmdata +
        rinfo[i * nparts_per_pe + k].nedges + rinfo[i * nparts_per_pe + k].nemdata;
  ...
  for(j = 0, i = 0; i < nparts; i++)
  {
    if(rinfo[i].nvtxs > 0)
    {
      ...
      gkMPI_Irecv((void*)(rgraph + gpwgts[i]), gpwgts[i + 1] - gpwgts[i], IDX_T, i % npes, 1 + i / npes, ctrl->comm,
                  ctrl->rreq + j++);
    }
    else
  ...
  for(j = 0, i = 0; i < nparts; i++)
  {
    if(sinfo[i].nvtxs > 0)
    {
      ...
      gkMPI_Isend((void*)(sgraph + lpwgts[i]), lpwgts[i + 1] - lpwgts[i], IDX_T, i / nparts_per_pe,
                  1 + i % nparts_per_pe, ctrl->comm, ctrl->sreq + j++);
    }
    else
  ...
  PASSERTP(
    ctrl, ii == gpwgts[nparts],
    (ctrl, "%" PRIDX " %" PRIDX " %" PRIDX " %" PRIDX " %" PRIDX "\n", ii, gpwgts[nparts], jj, mgraph->nedges, nvtxs));
  ...
  for(i = 0; i < nvtxs; i++)
  {
    for(j = xadj[i]; j < xadj[i + 1]; j++)
    {
      ...
      if(adjncy[j] >= firstvtx && adjncy[j] < lastvtx)
      {
        ...
        if(jj == xadj[k + 1])
          myprintf(ctrl,
                   "(%" PRIDX " %" PRIDX ") but not (%" PRIDX " %" PRIDX ") [%" PRIDX " %" PRIDX "] [%" PRIDX " %" PRIDX
                   "]\n",
...
graph_t* DistDGL_ReadGraph(char* fstem, MPI_Comm comm)
{
  ...
  /* ======================================================= */
  {
    ...
    */
    ...
    if(mype == 0)
    {
      ...
      gk_free((void**)&coo_buffers_cpos, &meta_buffers_cpos, &meta_buffers_len, &coo_buffers, &meta_buffers, LTERM);
    }
    ...
    for(j = 0, i = 1; i < lnedges; i++)
    {
      ...
      {
        if(lcoo[i].val != -1)
          printf("[%03" PRIDX "]Duplicate edges with metadata: %" PRIDX " [%s][%s]\n", mype, i, lmeta + lcoo[i].val,
                 (lcoo[j].val == -1 ? "NULL" : lmeta + lcoo[j].val));
      }
      else
      {
        lcoo[++j] = lcoo[i];
  ...
  /* ======================================================= */
  {
    ...
    if(mype == 0)
    {
      ...
      gk_free((void**)&con_buffers_cpos, &meta_buffers_cpos, &meta_buffers_len, &con_buffers, &meta_buffers, LTERM);
    }
    ...
    for(chunk = 0; chunk < nchunks; chunk++)
    {
      for(i = 0; i < con_chunks_len[chunk]; i++, nvtxs++)
      {
        ...
        vmptr[nvtxs + 1] =
          vmptr[nvtxs] + ((j + idxwidth - 1) / idxwidth) * idxwidth; /* pad it to idxwidth muptliples */
      }
        ...
        for(i = 0; i < graph->nvtxs; i++)
        {
          for(j = graph->xadj[i]; j < graph->xadj[i + 1]; j++)
          {
            ...
            printf("YYY%" PRIDX " [%" PRIDX " %" PRIDX "] [%" PRIDX " %" PRIDX "] vmdata: [%s]  emdata: [%s]\n", mype,
                   u, v, DistDGL_mapFromCyclic(u, npes, vtxdist), DistDGL_mapFromCyclic(v, npes, vtxdist),
                   graph->vmdata + graph->vmptr[i], graph->emdata + graph->emptr[j]);
          }
        }
        fflush(stdout);
      }
...
idx_t DistDGL_mapToCyclic(idx_t u, idx_t npes, idx_t* vtxdist) { return vtxdist[u % npes] + u / npes; }
...
#define ikeyval_lt(a, b)                                                                                               \
  ((a)->key1 < (b)->key1 || ((a)->key1 == (b)->key1 && (a)->key2 < (b)->key2) ||                                       \
