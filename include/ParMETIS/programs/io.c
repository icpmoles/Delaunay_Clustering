...
void ParallelReadGraph(graph_t* graph, char* filename, MPI_Comm comm)
{
  ...
  if(mype == npes - 1)
  {
    ...
    fmt = ncon = nobj = 0;
    sscanf(line, "%" PRIDX " %" PRIDX " %" PRIDX " %" PRIDX " %" PRIDX "", &gnvtxs, &gnedges, &fmt, &ncon, &nobj);
    gnedges *= 2;
    readew = (fmt % 10 > 0);
    readvw = ((fmt / 10) % 10 > 0);
 ...
 **************************************************************************/
void Mc_ParallelWriteGraph(ctrl_t* ctrl, graph_t* graph, char* filename, idx_t nparts, idx_t testset)
{
  idx_t h, i, j;
...
void Mc_SerialReadGraph(graph_t* graph, char* filename, idx_t* wgtflag, MPI_Comm comm)
{
  ...
  if(mype == 0)
  {
    ...
    Mc_SerialReadMetisGraph(filename, &nvtxs, &ncon, &nobj, &fmt, &gxadj, &gvwgt, &gadjncy, &gadjwgt, wgtflag);
    ...
    if(mype == 0)
    {
      for(penum = 0; penum < npes; penum++)
      {
        ...
        if(penum == mype)
          ...
          gkMPI_Send((void*)(gadjwgt + (gxadj[vtxdist[penum]] * nobj)), ssize[penum], IDX_T, penum, 1, comm);
      }
    }
    else
      gkMPI_Recv((void*)graph->adjwgt, graph->nedges * nobj, IDX_T, 0, 1, comm, &status);
  }
  else
  {
    ...
    if(mype == 0)
    {
      for(penum = 0; penum < npes; penum++)
      {
        ...
        if(penum == mype)
          ...
          gkMPI_Send((void*)(gvwgt + (vtxdist[penum] * ncon)), ssize[penum], IDX_T, penum, 1, comm);
      }

      gk_free((void**)&ssize, LTERM);
    }
    else
      gkMPI_Recv((void*)graph->vwgt, graph->nvtxs * ncon, IDX_T, 0, 1, comm, &status);
  }
  else
  {
 ...
 * This function reads the spd matrix
 **************************************************************************/
void Mc_SerialReadMetisGraph(char* filename, idx_t* r_nvtxs, idx_t* r_ncon, idx_t* r_nobj, idx_t* r_fmt, idx_t** r_xadj,
                             idx_t** r_vwgt, idx_t** r_adjncy, idx_t** r_adjwgt, idx_t* wgtflag)
{
  idx_t i, k, l;
...
void ParallelReadMesh(mesh_t* mesh, char* filename, MPI_Comm comm)
{
  ...
  if(mype == npes - 1)
  {
    ...
    for(i = 0; i < npes; i++)
    {
      maxnelms = (maxnelms > elmdist[i + 1] - elmdist[i]) ? maxnelms : elmdist[i + 1] - elmdist[i];
    }
