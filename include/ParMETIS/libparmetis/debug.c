...
void PrintVector2(ctrl_t* ctrl, idx_t n, idx_t first, idx_t* vec, char* title)
{
  ...
  for(penum = 0; penum < ctrl->npes; penum++)
  {
    if(ctrl->mype == penum)
    {
      ...
      for(i = 0; i < n; i++)
        printf("[%" PRIDX " %" PRIDX ".%" PRIDX "] ", first + i, (idx_t)(vec[i] >= KEEP_BIT ? 1 : 0),
...
void PrintGraph2(ctrl_t* ctrl, graph_t* graph)
{
  ...
  for(penum = 0; penum < ctrl->npes; penum++)
  {
    if(ctrl->mype == penum)
    {
      ...
      {
        if(i == 0)
          printf("\t%2" PRIDX " %2" PRIDX " [%" PRIDX " %" PRIDX " %" PRIDX "]\t", firstvtx + i, graph->vwgt[i],
                 graph->where[i], graph->ckrinfo[i].id, graph->ckrinfo[i].ed);
        else
          printf("\t\t%2" PRIDX " %2" PRIDX " [%" PRIDX " %" PRIDX " %" PRIDX "]\t", firstvtx + i, graph->vwgt[i],
                 graph->where[i], graph->ckrinfo[i].id, graph->ckrinfo[i].ed);
        for(j = graph->xadj[i]; j < graph->xadj[i + 1]; j++)
 ...
 **************************************************************************/
void PrintTransferedGraphs(ctrl_t* ctrl, idx_t nnbrs, idx_t* peind, idx_t* slens, idx_t* rlens, idx_t* sgraph,
                           idx_t* rgraph)
{
