      ...
      if(myrinfo->ed >= myrinfo->id)
      {
        ...
        for(j = k - 1; j >= 0; j--)
        {
          ...
          if((gain_is_greater && (fit_in_to || better_balance_ft)) ||
             (gain_is_same && ((fit_in_to && going_home) || better_balance_tt)))
          {
            k = j;
          }
        }
  ...
  { /* Do a number of passes */
    ...
    for(nswaps = 0; nswaps < nvtxs; nswaps++)
    {
      ...
      if((newcut < mincut && newbal - origbal <= .00001) ||
         (newcut == mincut &&
          (newbal < minbal || (newbal == minbal && Serial_BetterBalance(ncon, npwgts, tpwgts, mindiff, tmpdiff)))))
      {
        mincut = newcut;
        minbal = newbal;
        mincutorder = nswaps;
  ...
  *cnum = -1;
  ...
  if(maxdiff <= 0.0 || *from == -1)
  {
    ...
    for(part = 0; part < 2; part++)
    {
      for(i = 0; i < ncon; i++)
      {
        if(rpqLength(queues[part][i]) > 0 && rpqSeeTopKey(queues[part][i]) > maxgain)
        {
...
void Mc_Serial_Balance2Way(ctrl_t* ctrl, graph_t* graph, real_t* tpwgts, real_t lbfactor)
{
  ...
  for(nswaps = 0; nswaps < nvtxs; nswaps++)
  {
    ...
    if(newbal < minbal ||
       (newbal == minbal &&
        (newcut < mincut || (newcut == mincut && Serial_BetterBalance(ncon, npwgts, tpwgts, mindiff, tmpdiff)))))
    {
      mincut = newcut;
      minbal = newbal;
      mincutorder = nswaps;
