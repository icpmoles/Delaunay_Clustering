...
idx_t Mc_Diffusion(ctrl_t* ctrl, graph_t* graph, idx_t* vtxdist, idx_t* where, idx_t* home, idx_t npasses)
{
  ...
  for(pass = 0; pass < npasses; pass++)
  {
    ...
    while(nvisited < nlinks)
    {
      ...
      if(proc2sub[mype * 2] != UNMATCHED)
      {
        ...
        if(egraph != NULL)
        {
          ...
          if(ncon <= 4)
          {
            ...
            nswaps = BalanceMyLink(ctrl, egraph, ehome, me, you, diff_flows, maxdiff, &diff_cost, &diff_lbavg,
                                   1.0 / (real_t)nvtxs);

            if((sr_lbavg < diff_lbavg && (diff_lbavg >= ubavg - 1.0 || sr_cost == diff_cost)) ||
      ...
      gkMPI_Allgatherv((void*)&sbuffer[rdispl[mype]], rcount[mype], IDX_T, (void*)rbuffer, rcount, rdispl, IDX_T,
                       ctrl->comm);
