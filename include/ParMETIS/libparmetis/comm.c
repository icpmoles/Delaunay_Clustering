...
{
  idx_t i, j, k, islocal, penum, gnvtxs, nvtxs, nlocal, firstvtx, lastvtx, nsend, nrecv, nnbrs, nadj;
  ...
  for(i = 0; i < nnbrs; i++)
  {
    gkMPI_Irecv((void*)(sendind + sendptr[i]), sendptr[i + 1] - sendptr[i], IDX_T, peind[i], 1, ctrl->comm,
                ctrl->rreq + i);
  }
  ...
  for(i = 0; i < nnbrs; i++)
  {
    gkMPI_Isend((void*)(recvind + recvptr[i]), recvptr[i + 1] - recvptr[i], IDX_T, peind[i], 1, ctrl->comm,
                ctrl->sreq + i);
  }
  ...
  rprintf(ctrl,
          "[%5" PRIDX " %5" PRIDX "] \tl:[%5" PRIDX " %5" PRIDX "] \ts:[%5" PRIDX ", %5" PRIDX "] \tr:[%5" PRIDX
          ", %5" PRIDX "]\n",
          GlobalSEMin(ctrl, nvtxs), GlobalSEMax(ctrl, nvtxs), GlobalSEMin(ctrl, nlocal), GlobalSEMax(ctrl, nlocal),
          GlobalSEMin(ctrl, nsend), GlobalSEMax(ctrl, nsend), GlobalSEMin(ctrl, nrecv), GlobalSEMax(ctrl, nrecv));
...
void CommInterfaceData(ctrl_t* ctrl, graph_t* graph, idx_t* data, idx_t* recvvector)
{
  ...
  for(i = 0; i < nnbrs; i++)
  {
    gkMPI_Irecv((void*)(recvvector + recvptr[i]), recvptr[i + 1] - recvptr[i], IDX_T, peind[i], 1, ctrl->comm,
                ctrl->rreq + i);
  }
  ...
  for(i = 0; i < nnbrs; i++)
  {
    gkMPI_Isend((void*)(sendvector + sendptr[i]), sendptr[i + 1] - sendptr[i], IDX_T, peind[i], 1, ctrl->comm,
                ctrl->sreq + i);
  }
                              ...
                              ikv_t* sendpairs, ikv_t* recvpairs)
{
  ...
  idx_t *peind, *sendptr, *recvptr, *recvind, *pexadj, *peadjncy, *peadjloc, *psendptr;
  ikv_t* pairs;
  ...
  for(i = 0; i < nnbrs; i++)
  {
    gkMPI_Irecv((void*)(recvpairs + recvptr[i]), 2 * (recvptr[i + 1] - recvptr[i]), IDX_T, peind[i], 1, ctrl->comm,
                ctrl->rreq + i);
  }
  ...
  if(nchanged != 0)
  {
    ...
    for(i = 0; i < nnbrs; i++)
    {
      gkMPI_Isend((void*)(sendpairs + sendptr[i]), 2 * (psendptr[i] - sendptr[i]), IDX_T, peind[i], 1, ctrl->comm,
                  ctrl->sreq + i);
    }
