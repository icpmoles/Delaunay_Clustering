...
void Match_Global0(ctrl_t* ctrl, graph_t* graph)
{
  ...
  for(nmatched = pass = 0; pass < NMATCH_PASSES; pass++)
  {
    ...
    for(i = 0; i < nnbrs; i++)
    {
      gkMPI_Irecv((void*)(match_granted + recvptr[i]), 2 * (recvptr[i + 1] - recvptr[i]), IDX_T, peind[i], 1,
                  ctrl->comm, ctrl->rreq + i);
    }
    ...
    for(ii = 0; ii < nnbrs; ii++)
    {
      ...
      {
        k = pe_requests[j].key;
        PASSERTP(ctrl, k >= firstvtx && k < lastvtx,
        ...
        { /* Bingo, lets grant this request */
          ...
          if(nkept >= 0)
          { /* decide who to keep it based on local balance */
    ...
    for(i = 0; i < nnbrs; i++)
    {
      gkMPI_Irecv((void*)(match_requests + sendptr[i]), 2 * (sendptr[i + 1] - sendptr[i]), IDX_T, peind[i], 1,
                  ctrl->comm, ctrl->rreq + i);
    }
...
void CreateCoarseGraph_Global(ctrl_t* ctrl, graph_t* graph, idx_t cnvtxs)
{
  ...
  idx_t *match, *cmap;
  idx_t *cxadj, *cadjncy, *cadjwgt, *cvwgt, *cvsize = NULL, *chome = NULL, *cwhere = NULL, *cvtxdist;
  ...
  for(l = i = 0; i < nnbrs; i++)
  {
    ...
    if(rlens[i + 1] - rlens[i] > 0)
    {
      gkMPI_Irecv((void*)(rgraph + l), (4 + ncon) * (rlens[i + 1] - rlens[i]) + 2 * rsizes[i], IDX_T, peind[i], 1,
                  ctrl->comm, ctrl->rreq + i);
...
void CreateCoarseGraph_Local(ctrl_t* ctrl, graph_t* graph, idx_t cnvtxs)
{
  ...
  idx_t *match, *cmap;
  idx_t *cxadj, *cvwgt, *cvsize = NULL, *cadjncy, *cadjwgt, *cvtxdist, *chome = NULL, *cwhere = NULL;
  real_t* cnvwgt;
  graph_t* cgraph;
