 ...
 ************************************************************************************/
int ParMETIS_V3_PartKway(idx_t* vtxdist, idx_t* xadj, idx_t* adjncy, idx_t* vwgt, idx_t* adjwgt, idx_t* wgtflag,
                         ...
                         idx_t* edgecut, idx_t* part, MPI_Comm* comm)
{
  ...
  status = CheckInputsPartKway(vtxdist, xadj, adjncy, vwgt, adjwgt, wgtflag, numflag, ncon, nparts, tpwgts, ubvec,
  ...
  if(npes == 1)
  {
    ...
    status = METIS_PartGraphKway(&nvtxs, ncon, xadj, adjncy, vwgt, NULL, adjwgt, nparts, tpwgts, ubvec, moptions,
                                 edgecut, part);
...
void Global_Partition(ctrl_t* ctrl, graph_t* graph)
{
  ...
  {
    rprintf(ctrl, "[%6" PRIDX " %8" PRIDX " %5" PRIDX " %5" PRIDX "] [%" PRIDX "] [", graph->gnvtxs,
            GlobalSESum(ctrl, graph->nedges), GlobalSEMin(ctrl, graph->nvtxs), GlobalSEMax(ctrl, graph->nvtxs),
            ctrl->CoarsenTo);
    for(i = 0; i < ncon; i++)
      rprintf(ctrl, " %.3" PRREAL "",
              GlobalSEMinFloat(ctrl, graph->nvwgt[rargmin_strd(graph->nvtxs, graph->nvwgt + i, ncon) * ncon + i]));
    rprintf(ctrl, "] [");
    for(i = 0; i < ncon; i++)
      rprintf(ctrl, " %.3" PRREAL "",
              GlobalSEMaxFloat(ctrl, graph->nvwgt[rargmax_strd(graph->nvtxs, graph->nvwgt + i, ncon) * ncon + i]));
    rprintf(ctrl, "]\n");
  }
