...
void Balance_Partition(ctrl_t* ctrl, graph_t* graph)
{
  ...
  { /* Half of the processors do scratch-remap */
    ...
    while(lnpes > 1 && lnparts > 1)
    {
      ...
      METIS_PartGraphRecursive(&agraph->nvtxs, &ncon, agraph->xadj, agraph->adjncy, agraph->vwgt, NULL, agraph->adjwgt,
    ...
    if(lnparts == 1)
    { /* Case in which srnpes is greater or equal to nparts */
    ...
    { /* Case in which srnpes is smaller than nparts */
      ...
      METIS_PartGraphKway(&agraph->nvtxs, &ncon, agraph->xadj, agraph->adjncy, agraph->vwgt, NULL, agraph->adjwgt,
                          &lnparts, tpwgts, NULL, moptions, &edgecut, part);
  ...
  if(graph->ncon <= MAX_NCON_FOR_DIFFUSION)
  {
    ...
    {
      who_wins = gd_pe;
      if((my_balance < 1.1 && your_balance > 1.1) || (my_balance < 1.1 && your_balance < 1.1 && my_cost < your_cost) ||
