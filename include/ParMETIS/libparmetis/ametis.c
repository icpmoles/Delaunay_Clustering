 ...
 ************************************************************************************/
int ParMETIS_V3_AdaptiveRepart(idx_t* vtxdist, idx_t* xadj, idx_t* adjncy, idx_t* vwgt, idx_t* vsize, idx_t* adjwgt,
                               idx_t* wgtflag, idx_t* numflag, idx_t* ncon, idx_t* nparts, real_t* tpwgts,
                               real_t* ubvec, real_t* ipc2redist, idx_t* options, idx_t* edgecut, idx_t* part,
                               MPI_Comm* comm)
{
  ...
  status = CheckInputsAdaptiveRepart(vtxdist, xadj, adjncy, vwgt, vsize, adjwgt, wgtflag, numflag, ncon, nparts, tpwgts,
  ...
  ctrl->CoarsenTo =
    gk_min(graph->gnvtxs + 1, (gk_max(npes, *nparts) > 256 ? 20 : 50) * (*ncon) * gk_max(npes, *nparts));
...
void Adaptive_Partition(ctrl_t* ctrl, graph_t* graph)
{
  ...
  gtewgt =
    (real_t)GlobalSESum(ctrl, tewgt) + 1.0 / graph->gnvtxs; /* The +1/graph->gnvtxs were added to remove any FPE */
  ...
  IFSET(ctrl->dbglvl, DBG_PROGRESS,
        rprintf(ctrl, "[%6" PRIDX " %8" PRIDX " %5" PRIDX " %5" PRIDX "][%" PRIDX "]\n", graph->gnvtxs,
