...
{
  idx_t i, ii, iii, j, jj, k, pass, nvtxs, nrecv, firstvtx, lastvtx, otherlastvtx, side, c, cc, nmoves, nlupd, nsupd,
    nnbrs, nchanged, nsep, nzerogainiterations;
  ...
  for(pass = 0; pass < npasses; pass++)
  {
    ...
    for(side = 0; side < 2; side++)
    {
      ...
      IFSET(ctrl->dbglvl, DBG_RMOVEINFO,
            rprintf(ctrl, "\t[%" PRIDX " %" PRIDX "], [%" PRIDX " %" PRIDX " %" PRIDX "]\n", pass, c,
...
void PrintNodeBalanceInfo(ctrl_t* ctrl, idx_t nparts, idx_t* gpwgts, idx_t* badmaxpwgt, char* title)
{
  ...
  if(ctrl->mype == 0)
  {
    ...
    for(i = 0; i < nparts; i += 2)
      printf(" [%5" PRIDX " %5" PRIDX " %5" PRIDX " %5" PRIDX "]", gpwgts[i], gpwgts[i + 1], gpwgts[nparts + i],
             badmaxpwgt[i]);
    printf("\n");
  }
