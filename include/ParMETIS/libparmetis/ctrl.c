                  ...
                  MPI_Comm comm)
{
  ...
  case PARMETIS_OP_RMETIS:
    ...
    ctrl->ps_relation = (defopts ? (ctrl->npes == nparts ? PARMETIS_PSR_COUPLED : PARMETIS_PSR_UNCOUPLED)
                                 : (ctrl->npes == nparts ? options[PMV3_OPTION_PSR] : PARMETIS_PSR_UNCOUPLED));
    break;
...
void FreeCtrl(ctrl_t** r_ctrl)
{
  ...
  gk_free((void**)&ctrl->invtvwgts, &ctrl->ubvec, &ctrl->tpwgts, &ctrl->sreq, &ctrl->rreq, &ctrl->statuses, &ctrl,
          LTERM);
