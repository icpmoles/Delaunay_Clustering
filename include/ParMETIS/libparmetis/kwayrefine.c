...
#define ProperSide(c, from, other) (((c) == 0 && (from) - (other) < 0) || ((c) == 1 && (from) - (other) > 0))
...
void ProjectPartition(ctrl_t* ctrl, graph_t* graph)
{
  ...
  if(graph->match_type == PARMETIS_MTYPE_GLOBAL)
  { /* Only if global matching is on */
    ...
    for(i = 0; i < nnbrs; i++)
    {
      if(slens[i + 1] - slens[i] > 0) /* Issue a receive only if you are getting something */
        gkMPI_Irecv((void*)(scand + slens[i]), 2 * (slens[i + 1] - slens[i]), IDX_T, peind[i], 1, ctrl->comm,
                    ctrl->rreq + i);
    }
    ...
    for(i = 0; i < nnbrs; i++)
    {
      if(rlens[i + 1] - rlens[i] > 0) /* Issue a send only if you are sending something */
        gkMPI_Isend((void*)(rcand + rlens[i]), 2 * (rlens[i + 1] - rlens[i]), IDX_T, peind[i], 1, ctrl->comm,
                    ctrl->sreq + i);
    }
  }
...
void ComputePartitionParams(ctrl_t* ctrl, graph_t* graph)
{
  ...
  rprintf(ctrl, "where sum: %" PRIDX " %" PRIDX " %" PRIDX " %" PRIDX " %" PRIDX "\n",
          GlobalSESum(ctrl, nvtxs + graph->nrecv), GlobalSESum(ctrl, isum(nvtxs + graph->nrecv, where, 1)),
          GlobalSESum(ctrl, isum(nvtxs + 1, xadj, 1)), GlobalSESum(ctrl, isum(graph->nedges, adjncy, 1)),
    ...
    for(c = 0; c < 2; c++)
    {
      ...
      for(iii = 0; iii < nvtxs; iii++)
      {
        ...
        if(mynbrs[k].ed > myrinfo->id ||
           (mynbrs[k].ed == myrinfo->id && (imbalanced || graph->level > 3 || iii % 8 == 0) &&
      ...
      for(j = 0; j < nparts; j++)
      {
        for(h = 0; h < ncon; h++)
        {
          ...
          {
            overfill[j * ncon + h] =
              (pgnpwgts[j * ncon + h] - badmaxpwgt[j * ncon + h]) / (pgnpwgts[j * ncon + h] - ognpwgts[j * ncon + h]);
          }
          else
          {
      ...
      IFSET(ctrl->dbglvl, DBG_RMOVEINFO,
            rprintf(ctrl, "\t[%" PRIDX " %" PRIDX "], [%.4" PRREAL "], [%" PRIDX " %" PRIDX " %" PRIDX "]\n", pass, c,
                    badmaxpwgt[0], GlobalSESum(ctrl, nmoves), GlobalSESum(ctrl, nsupd), GlobalSESum(ctrl, nlupd)));
      ...
      for(i = 0; i < nnbrs; i++)
      {
        gkMPI_Irecv((void*)(rupdate + sendptr[i]), sendptr[i + 1] - sendptr[i], IDX_T, peind[i], 1, ctrl->comm,
                    ctrl->rreq + i);
      }
