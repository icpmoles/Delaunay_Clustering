...
namespace CLS
{
  ...
  inline void Mesh_Augmented::iterate()
  {
    ...
    Face_handle first_face = get_inlier_face_();
    std::cout << "triangle of interest:" << std::endl;
    std::cout << "1) " << first_face->vertex(0)->point() << std::endl;
    std::cout << "2) " << first_face->vertex(1)->point() << std::endl;
    std::cout << "3) " << first_face->vertex(2)->point() << std::endl;
    ...
    do {
      for (const Vertex_handle v : this->Clusters_[cluster_id])
      {
        ...
        do
        {
          std::cout << vertex_counter << "th vertex:" << circ->point() << std::endl;
          vertex_counter++;
        }
  ...
  inline Face_handle Mesh_Augmented::get_inlier_face_()
  {
