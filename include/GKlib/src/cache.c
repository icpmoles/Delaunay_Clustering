...
int gk_cacheLoad(gk_cache_t* cache, size_t addr)
{
  ...
  // printf("%16"PRIx64" %16"PRIx64" %16"PRIx64" ", (uint64_t)addr, (uint64_t)addr&(cache->cmask),
  // (uint64_t)cache->cmask);
  ...
  for(i = 0; i < nway; i++)
  { /* look for empty spots or the lru spot */
    if(clines[i] == 0)
    {
      lru = i;
      break;
 ...
 */
/*************************************************************************/
double gk_cacheGetHitRate(gk_cache_t* cache) { return ((double)cache->nhits) / ((double)(cache->clock + 1)); }
