...
void gk_find_frequent_itemsets(
  ...
  void (*process_itemset)(void* stateptr, int nitems, int* itemids, int ntrans, int* transids), void* stateptr)
{
  ssize_t i;
  gk_csr_t *mat, *pmat;
  isparams_t params;
...
void itemsets_find_frequent_itemsets(isparams_t* params, gk_csr_t* mat, int preflen, int* prefix)
{
  ...
  for(i = 0; i < mat->ncols; i++)
  {
    ...
    if(preflen + 1 >= params->minlen)
      (*params->callback)(params->stateptr, preflen + 1, prefix, mat->colptr[i + 1] - mat->colptr[i],
