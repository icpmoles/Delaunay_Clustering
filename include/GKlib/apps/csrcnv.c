...
/*! Local variables */
/*************************************************************************/
static struct gk_option long_options[] = {{"numone", 0, 0, CMD_NUMONE},
...
static char shorthelpstr[][100] = {" ", "   Usage: csrconv [options] <infile> <inf> <outfile> <outf>",
...
params_t* parse_cmdline(int argc, char* argv[])
{
  ...
  while((c = gk_getopt_long_only(argc, argv, "", long_options, &option_index)) != -1)
  {
    ...
    case CMD_MINCOLFREQ:
      if(gk_optarg)
        params->mincolfreq = atoi(gk_optarg);
      break;
    case CMD_MINROWFREQ:
      if(gk_optarg)
        params->minrowfreq = atoi(gk_optarg);
      break;
    case CMD_MAXCOLFREQ:
      if(gk_optarg)
        params->maxcolfreq = atoi(gk_optarg);
      break;
    case CMD_MAXROWFREQ:
      if(gk_optarg)
        params->maxrowfreq = atoi(gk_optarg);
      break;
    case CMD_ROWNRMFLTR:
      if(gk_optarg)
        params->rownrmfltr = atof(gk_optarg);
      break;
...
int main(int argc, char* argv[])
{
  ...
  if(params->mincolfreq != -1 || params->maxcolfreq != -1)
  {
    ...
    printf("Column prune: %d %d; nnz: %zd => ", params->mincolfreq, params->maxcolfreq, mat->rowptr[mat->nrows]);
  ...
  if(params->minrowfreq != -1 || params->maxrowfreq != -1)
  {
    ...
    printf("Row prune: %d %d; nnz: %zd => ", params->minrowfreq, params->maxrowfreq, mat->rowptr[mat->nrows]);
  ...
  if(params->srenumber)
  {
    ...
    if(gk_i32max(nlines, iperm, 1) == nlines)
    { /* need to renumber */
      for(i = 0; i < nlines; i++)
        iperm[i]--;
    }
