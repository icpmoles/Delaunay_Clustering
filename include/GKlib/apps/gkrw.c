...
/*! Local variables */
/*************************************************************************/
static struct gk_option long_options[] = {{"niter", 1, 0, CMD_NITER},
                                          {"lamda", 1, 0, CMD_LAMDA},
                                          {"eps", 1, 0, CMD_EPS},
                                          {"ppr", 1, 0, CMD_PPR},
                                          {"ntvs", 1, 0, CMD_NTVS},
...
static char shorthelpstr[][100] = {" ", "   Usage: rw [options] <graph-file> <out-file>",
...
int main(int argc, char* argv[])
{
  ...
  else
  {
    ...
    for(i = 0; i < mat->nrows; i++)
    {
      ...
      fprintf(fpout, "%.4e %10zd %.4e %10d\n", pr[i], mat->rowptr[i + 1] - mat->rowptr[i], maxdiff, jmax + 1);
    }
    gk_fclose(fpout);
...
void print_init_info(params_t* params, gk_csr_t* mat)
{
  ...
  printf("\n");
  printf("Options --------------------------------------------------------------------\n");
  printf(" niter=%d, ntvs=%d, ppr=%d, lamda=%f, eps=%e\n", params->niter, params->ntvs, params->ppr, params->lamda,
         params->eps);
...
params_t* parse_cmdline(int argc, char* argv[])
{
  ...
  while((c = gk_getopt_long_only(argc, argv, "", long_options, &option_index)) != -1)
  {
    ...
    case CMD_NTVS:
      if(gk_optarg)
        params->ntvs = atoi(gk_optarg);
      break;
    case CMD_PPR:
      if(gk_optarg)
        params->ppr = atoi(gk_optarg);
      break;
    case CMD_EPS:
      if(gk_optarg)
