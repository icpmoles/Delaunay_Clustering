...
/*! Local variables */
/*************************************************************************/
static struct gk_option long_options[] = {{"lnbits", 1, 0, CMD_LNBITS}, {"cnbits", 1, 0, CMD_CNBITS},
                                          {"type", 1, 0, CMD_TYPE},     {"niter", 1, 0, CMD_NITER},
                                          {"lamda", 1, 0, CMD_LAMDA},   {"eps", 1, 0, CMD_EPS},
                                          {"nosort", 0, 0, CMD_NOSORT}, {"write", 0, 0, CMD_WRITE},
                                          {"help", 0, 0, CMD_HELP},     {0, 0, 0, 0}};
...
static char shorthelpstr[][100] = {" ", "   Usage: gkgraph [options] <graph-file> [<out-file>]",
...
void test_spmv(params_t* params)
{
  ...
  if(params->write)
    gk_graph_Write(graph, "original.ijv", GK_GRAPH_FMT_IJV, 1);
  ...
  if(params->write)
    gk_graph_Write(pgraph, "bfs.ijv", GK_GRAPH_FMT_IJV, 1);
  ...
  sort_adjacencies(params, pgraph);
  if(params->write)
    gk_graph_Write(pgraph, "degrees.ijv", GK_GRAPH_FMT_IJV, 1);
  ...
  sort_adjacencies(params, pgraph);
  if(params->write)
    gk_graph_Write(pgraph, "freqlpn.ijv", GK_GRAPH_FMT_IJV, 1);
  ...
  sort_adjacencies(params, pgraph);
  if(params->write)
    gk_graph_Write(pgraph, "freqlpn-db.ijv", GK_GRAPH_FMT_IJV, 1);
  ...
  sort_adjacencies(params, pgraph);
  if(params->write)
    gk_graph_Write(pgraph, "minlpn.ijv", GK_GRAPH_FMT_IJV, 1);
  ...
  sort_adjacencies(params, pgraph);
  if(params->write)
    gk_graph_Write(pgraph, "minlpn-db.ijv", GK_GRAPH_FMT_IJV, 1);
...
double compute_tcstats(params_t* params, gk_graph_t* graph, int32_t* iperm)
{
  ...
  for(ntriangles = 0, vjj = 0; vjj < nvtxs; vjj++)
  {
    ...
    for(ej = uxadj[vj], ejend = xadj[vj + 1]; ej < ejend; ej++)
    {
      ...
      vk = adjncy[ej];
      for(l = (vk & hmsize); gk_cacheLoad(cache, (size_t)(&hmap[l])) && hmap[l] != 0; l = ((l + 1) & hmsize))
        ;
      hmap[l] = vk;
    }
    ...
    for(ej = xadj[vj], ejend = uxadj[vj]; ej < ejend; ej++)
    {
      ...
      {
        vk = adjncy[ei];
        for(l = vk & hmsize; gk_cacheLoad(cache, (size_t)(&hmap[l])) && hmap[l] != 0 && hmap[l] != vk;
            l = ((l + 1) & hmsize))
          ;
    ...
    for(ej = uxadj[vj], ejend = xadj[vj + 1]; ej < ejend; ej++)
    {
      gk_cacheLoad(cache, (size_t)(&adjncy[ej]));
      vk = adjncy[ej];
      for(l = (vk & hmsize); gk_cacheLoad(cache, (size_t)(&hmap[l])) && hmap[l] != vk; l = ((l + 1) & hmsize))
        ;
      hmap[l] = 0;
    }
  }
...
void print_init_info(params_t* params, gk_graph_t* graph)
{
  ...
  printf("\n");
  printf("Options --------------------------------------------------------------------\n");
  printf(" lnbits=%d, cnbits=%d, type=%d, niter=%d, lamda=%f, eps=%e\n", params->lnbits, params->cnbits, params->type,
         params->niter, params->lamda, params->eps);
...
params_t* parse_cmdline(int argc, char* argv[])
{
  ...
  while((c = gk_getopt_long_only(argc, argv, "", long_options, &option_index)) != -1)
  {
    ...
    case CMD_LNBITS:
      if(gk_optarg)
        params->lnbits = atoi(gk_optarg);
      break;
    case CMD_CNBITS:
      if(gk_optarg)
        params->cnbits = atoi(gk_optarg);
      break;
    case CMD_TYPE:
      if(gk_optarg)
        params->type = atoi(gk_optarg);
      break;
    case CMD_NITER:
      if(gk_optarg)
    ...
    case CMD_EPS:
      if(gk_optarg)
        params->eps = atof(gk_optarg);
      break;
    case CMD_LAMDA:
      if(gk_optarg)
        params->lamda = atof(gk_optarg);
      break;
    case CMD_NOSORT:
      params->nosort = 1;
      break;
