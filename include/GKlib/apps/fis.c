...
/*! Local variables */
/*************************************************************************/
static struct gk_option long_options[] = {{"minlen", 1, 0, CMD_MINLEN},   {"maxlen", 1, 0, CMD_MAXLEN},
                                          {"minfreq", 1, 0, CMD_MINFREQ}, {"maxfreq", 1, 0, CMD_MAXFREQ},
                                          {"silent", 0, 0, CMD_SILENT},   {"clabels", 1, 0, CMD_CLABELFILE},
...
static char shorthelpstr[][100] = {" ", "   Usage: fis [options] <mat-file>",
...
int main(int argc, char* argv[])
{
  ...
  gk_find_frequent_itemsets(mat->nrows, mat->rowptr, mat->rowind, params->minfreq, params->maxfreq, params->minlen,
                            params->maxlen, &print_an_itemset, (void*)params);
...
params_t* parse_cmdline(int argc, char* argv[])
{
  ...
  while((c = gk_getopt_long_only(argc, argv, "", long_options, &option_index)) != -1)
  {
    ...
    case CMD_MINLEN:
      if(gk_optarg)
        params->minlen = atoi(gk_optarg);
      break;
    case CMD_MAXLEN:
      if(gk_optarg)
        params->maxlen = atoi(gk_optarg);
      break;
    case CMD_MINFREQ:
      if(gk_optarg)
        params->minfreq = atoi(gk_optarg);
      break;
    case CMD_MAXFREQ:
      if(gk_optarg)
        params->maxfreq = atoi(gk_optarg);
      break;
    ...
    case CMD_CLABELFILE:
      if(gk_optarg)
        params->clabelfile = gk_strdup(gk_optarg);
      break;
...
/*************************************************************************/
void print_an_itemset(void* stateptr, int nitems, int* itemids, int ntrans, int* transids)
{
  ssize_t i;
  params_t* params;
